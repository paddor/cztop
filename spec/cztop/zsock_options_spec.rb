...
describe CZTop::ZsockOptions do
  ...
  describe CZTop::ZsockOptions::OptionsAccessor do
    ...
    describe '#heartbeat_ivl' do
      before { skip 'requires ZMQ 4.2+ and CZMQ drafts' unless has_zmq_version?('4.2') && has_czmq_drafts? }

      describe 'with no IVL' do
        it 'returns zero' do
          assert_equal 0, socket.options.heartbeat_ivl
        end
      end
      describe 'with IVL set' do
        let(:ivl) { 5 }
    ...
    describe '#heartbeat_ttl' do
      ...
      describe 'with no TTL' do
        it 'returns zero' do
          assert_equal 0, socket.options.heartbeat_ttl
        end
      end
      describe 'with TTL set' do
        let(:ttl) { 500 }
      ...
      describe 'with invalid TTL' do
        it 'raises' do
          assert_raises(ArgumentError) { socket.options.heartbeat_ttl = 500.3 }
        end
      end
      describe 'with out-of-range TTL' do
        it 'raises' do
          assert_raises(ArgumentError) { socket.options.heartbeat_ttl = 100_000 }
        end
      end
      describe 'with insignificant TTL' do
        it 'has no effect' do
          socket.options.heartbeat_ttl = 80 # less than 100
          assert_equal 0, socket.options.heartbeat_ttl
        end
      end
    end
    describe '#heartbeat_timeout' do
      ...
      describe 'integration test' do
        ...
        describe 'with client connected' do
          before do
            server_socket
            server_mon
            client_socket
        ...
        describe 'with client socket dead' do
          before do
            server_socket
            server_mon
            client_socket
